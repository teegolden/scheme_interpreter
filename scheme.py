...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        "*** YOUR CODE HERE ***"
        func_frame = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, func_frame)
    elif isinstance(procedure, MuProcedure):
        "*** YOUR CODE HERE ***"
        func_frame = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, func_frame)
    else:
...
def apply_primitive(procedure, args, env):
    ...
    for i in range(0, len(args)):
        procedure_args.append(args[i])
    if procedure.use_env == True:
        procedure_args.append(env)
    try:
        return procedure.fn(*procedure_args)
    except TypeError:
        raise SchemeError("Error evaluating {0}".format(procedure.fn))
...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if formals_len != len(vals):
            raise SchemeError("function has a different number of formals and values")
        for i in range(0, formals_len):
...
def do_lambda_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    if len(vals) > 2:
        # make it into a begin expression
        return LambdaProcedure(formals, Pair("begin", vals.second), env)
    else:
        # return a lambda expression object
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        check_form(vals, 2, 2)
        "*** YOUR CODE HERE ***"
        env.bindings[target] = scheme_eval(vals[1], env)       #maybe check primitive?
        return target[0]
    elif isinstance(target, Pair):
        "*** YOUR CODE HERE ***"
        if (type(target[0]) == str and target[0].islower()):
            env.bindings[target[0]] = do_lambda_form(Pair(target.second, vals.second), env)
            return target[0]
        else:
            raise SchemeError("cannot assign a function to a number")
    else:
...
def do_and_form(vals, env):
    ...
    if last_index < 0:  
        return True     
    for k in range(last_index):    # checks all but the last value
...
def do_or_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    last_index = len(vals) - 1
    if last_index < 0:  # if vals is an empty list false is returned
        return False
    for k in range(last_index):    # checks all but the last value
        if scheme_true(scheme_eval(vals[k], env)):
            return scheme_eval(quote(vals[k]), env) #check!!!!!!!!
    return vals[last_index]
...
def do_begin_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    vals_len = len(vals)
    for i in range(0, vals_len - 1):
        scheme_eval(vals[i], env)
    return vals[vals_len - 1]
...
def check_formals(formals):
    ...
    formals_list = []
    for x in formals:
        if x in formals_list:
            raise SchemeError('repeated symbol' + str(x))
        if not scheme_symbolp(x):
            raise SchemeError(str(x) + ' is not a valid symbol')
        formals_list.append(x)
