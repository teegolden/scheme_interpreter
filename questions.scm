...
(define (list-partitions total max-pieces max-value)
  ; *** YOUR CODE HERE ***
        (define (make_partitions total max-pieces max-value partitions current_patitions)
                (cond ((< max-pieces 0) '())
                      ((= total 0) (cons current_patitions partitions))
                      ((<= max-value 0) '())
                      (else (append (make_partitions (- total max-value) (- max-pieces 1) max-value partitions (append current_patitions (list max-value))) 
                                          (make_partitions total max-pieces (- max-value 1) partitions current_patitions)))))
        (make_partitions total max-pieces max-value '() '()))
...
(define (tree-to-lst tree)
    (cons (entry tree) (map tree-to-lst (children tree))))

(define (tree-sums tree)
  ; *** YOUR CODE HERE ***
  (cond ((null? tree) 0)
        ((null? (children tree)) (list (entry tree)))
        (else (map (lambda (node) (+ node (entry tree))) (flatten(map tree-sums (children tree)))))))
